...
export default function BusinessPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* Categories */}
      <div className="flex gap-2 px-4 py-2.5 bg-white overflow-x-auto scrollbar-hide border-b border-gray-100">
        {CATEGORIES.map((cat) => (
          <button key={cat.key}
        ...
        <div className="p-4 space-y-3">
          {filteredPlaces.map((place) => (
            <Link key={place.place_id} href={`/business/${place.place_id}?lat=${location.latitude}&lng=${location.longitude}`}
              ...
              {place.photo_url ? (
                <img src={place.photo_url} alt={place.name} className="w-24 h-[100px] object-cover shrink-0" />
              ) : (
                <div className="w-24 h-[100px] bg-gray-100 flex items-center justify-center shrink-0">
