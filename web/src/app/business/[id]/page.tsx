  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-8">
      <div className="max-w-4xl mx-auto">
        ...
        {detail.photos && detail.photos.length > 0 ? (
          <div className="flex overflow-x-auto scrollbar-hide">
            {detail.photos.map((url: string, i: number) => (
              <img key={i} src={url} alt={`Photo ${i + 1}`} className="w-full md:w-1/2 h-52 md:h-72 object-cover shrink-0" />
            ))}
          </div>
        ) : (
          <div className="w-full h-52 md:h-72 bg-gray-200 flex items-center justify-center">
        ...
        <div className="md:grid md:grid-cols-2 md:gap-6 md:px-6 md:mt-4">
          {/* Left Column */}
          <div>
            {/* Info Section */}
            <div className="bg-white p-5 md:rounded-xl md:shadow-sm space-y-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {distance && (
                  <span className="text-sm font-bold text-[#FF6B35] bg-[#FF6B35]/10 px-2.5 py-1 rounded-full">📍 {distance}</span>
                )}
              </div>
              {detail.is_open !== null && (
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600"><span>📍</span><span>{detail.address}</span></div>
              {detail.phone && (
                <a href={`tel:${detail.phone}`} className="flex items-center gap-2 text-sm text-[#FF6B35]"><span>📞</span><span>{detail.phone}</span></a>
              )}
              {detail.website && (
                <a href={detail.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-[#FF6B35] truncate">
                  <span>🌐</span><span className="truncate">{detail.website}</span>
                </a>
              )}
            </div>
            ...
            {detail.opening_hours && detail.opening_hours.length > 0 && (
              <>
                <h3 className="text-base font-bold text-gray-900 px-4 md:px-0 mb-2">영업 시간</h3>
                <div className="mx-4 md:mx-0 bg-white rounded-xl p-4 shadow-sm mb-4 space-y-1.5">
          ...
          <div>
            {/* Map */}
            {detail.google_maps_url && (
              <div className="mx-4 md:mx-0 bg-white rounded-xl overflow-hidden shadow-sm mb-4">
                <div className="h-44 md:h-64 bg-gray-100">
                  <iframe
            ...
            {/* Reviews */}
            <h3 className="text-base font-bold text-gray-900 px-4 md:px-0 mb-2">리뷰</h3>
            {detail.reviews && detail.reviews.length > 0 ? (
              ...
              <div className="mx-4 md:mx-0 bg-white rounded-xl p-6 shadow-sm">
