    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between px-5 pt-6 pb-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#FF6B35]">PetAI</h1>
        </div>
      ...
      {/* Banner */}
      <div className="mx-4 mb-5 rounded-2xl bg-[#FF6B35] p-5 flex items-center justify-between overflow-hidden">
        <div className="flex-1">
          <h2 className="text-lg font-bold text-white mb-1">AI 반려동물 상담</h2>
          <p className="text-sm text-white/85 leading-5 mb-3">건강, 영양, 행동 등{'\n'}무엇이든 물어보세요</p>
          <Link href="/chat" className="inline-flex items-center gap-1.5 bg-white/20 text-white text-sm font-semibold px-4 py-2 rounded-full">
            상담 시작 →
          </Link>
        </div>
        <div className="text-6xl opacity-30 ml-3">🐾</div>
      </div>
      ...
      <div className="flex justify-around px-4 mb-6">
        {QUICK_MENU.map((item) => (
          <Link key={item.label} href={item.href} className="flex flex-col items-center gap-2">
            <div className={`w-14 h-14 rounded-xl ${item.bg} flex items-center justify-center text-2xl`}>
              {item.icon}
            </div>
      ...
      <div className="flex items-center justify-between px-5 mb-3">
        <h3 className="text-base font-bold text-gray-900">서비스 찾기</h3>
      ...
      <div className="flex gap-4 px-4 pb-4 overflow-x-auto scrollbar-hide">
        {SERVICE_CATEGORIES.map((item) => (
          <Link key={item.category} href={`/business?cat=${item.category}`} className="flex flex-col items-center min-w-[60px] gap-1.5">
      ...
      <div className="flex items-center justify-between px-5 mb-3">
        <h3 className="text-base font-bold text-gray-900">최근 상담</h3>
        <Link href="/consultation" className="text-sm text-[#FF6B35]">더보기</Link>
      </div>
      <div className="mx-4 mb-5 bg-white rounded-xl p-6 shadow-sm">
      ...
      <div className="flex gap-3 px-4 pb-8 overflow-x-auto scrollbar-hide">
        {POPULAR_PRODUCTS.map((name) => (
          <Link key={name} href="/shopping" className="min-w-[130px] bg-white rounded-xl shadow-sm overflow-hidden">
