    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="mx-4 mt-4 bg-white rounded-xl p-5 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-[#FF6B35] flex items-center justify-center shrink-0">
      ...
      <div className="mx-4 mt-3 bg-white rounded-xl shadow-sm overflow-hidden">
        {MENU_ITEMS.map((item, i) => (
          <Link key={item.label} href={item.href}
            className={`flex items-center gap-3 px-4 py-3.5 ${i < MENU_ITEMS.length - 1 ? 'border-b border-gray-50' : ''}`}>
      ...
      {/* Logout */}
      <button onClick={handleLogout} className="w-full text-center py-4 mt-4 text-sm text-red-500 font-medium">
        로그아웃
      </button>
    </div>
  );
